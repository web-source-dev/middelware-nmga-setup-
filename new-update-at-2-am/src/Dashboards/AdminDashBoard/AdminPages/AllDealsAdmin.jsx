...
const AllDealsAdmin = () => {
  ...
  const fetchDistributors = async () => {
    try {
      const response = await api.get(`/api/users/distributor-list`);
  ...
  const fetchDeals = async () => {
    try {
      setLoading(true);
      let url = `/deals/allDeals/admin-all-deals?page=${page}&limit=${limit}`;
  ...
  const handleViewCommitments = async (deal) => {
    try {
      const response = await api.get(`/deals/allDeals/deal-commitments-admin/${deal._id}`);
      setSelectedDeal({
        ...deal,
    ...
    handleConfirmDialogOpen(
      ...
      async () => {
        try {
          ...
          const endpoint = action === 'approve' ? 'bulk-approve-commitments-admin' : 'bulk-decline-commitments-admin';
          
          await api.post(`/deals/allDeals/${endpoint}`, {
            dealId,
      ...
      async () => {
        try {
          ...
          await Promise.all(selectedDeals.map(dealId => {
            const deal = deals.find(d => d._id === dealId);
            return api.post(`/deals/allDeals/${endpoint}`, {
              dealId,
              distributorId: deal.distributor._id
            });
          }));
