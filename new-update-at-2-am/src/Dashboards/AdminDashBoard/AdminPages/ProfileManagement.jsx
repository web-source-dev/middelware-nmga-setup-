...
const ProfileManagement = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        // Determine which profile to fetch based on current context
        let profileUserId = currentUserId;
        
        // If admin is impersonating, use the impersonated user's ID
        if (isImpersonating) {
          profileUserId = currentUserId; // This will be the impersonated user's ID
        }
        
        // Fetch user profile based on role
        let response;
          response = await api.get('/member/user');
    ...
    const fetchLogs = async () => {
      try {
        // Fetch logs for the current user (or impersonated user)
        const response = await api.get('/common/logs/user');
  ...
  }, [currentUserId, isImpersonating, userRole, location.search]);
    ...
    <Container maxWidth="xl" sx={{ minWidth: { xs: '100%', sm: '300px' } }}>
      ...
      <Card elevation={3} sx={{ mt: 2, p: 3, position: 'relative' }}>
        ...
        >
          ...
          <Tab label="Settings" />
          {userRole === 'distributor' && <Tab label="Deals Management" />}
        </Tabs>
    ...
    <>
      ...
      {user.additionalPhoneNumbers?.length > 0 && (
        ...
        <Box hidden={tabIndex !== 2} sx={{ p: 2 }}> {/* Settings Tab Content */}
          <MemberSettings /> {/* Call UserSettings component */}
        </Box>
        {userRole === 'distributor' && (
          <Box hidden={tabIndex !== 3} sx={{ p: 2 }}> {/* Deals Management Tab Content */}
            <ManageDeals /> {/* Call ManageDeals component */}
          </Box>
        )}
        <CardActions sx={{ position: 'absolute', top: '1rem', right: '1rem' }}>
          {/* Only show admin actions if user is admin and not impersonating */}
